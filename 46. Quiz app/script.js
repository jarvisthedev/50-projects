...
const renderAnswer = event => {
  Array.from(answers).map(el => el.classList.remove('ans--correct'));
  event.classList.add('ans--correct');
  if (event.classList.contains('correct')) current_score += 1;
  questionNum += 1;
...
container.addEventListener('click', function (e) {
  ...
  if (clicked.classList.contains('ans')) renderAnswer(clicked);
